...
export const BeforeAfterViewport: React.FC<BeforeAfterViewportProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full w-full bg-background-surface relative select-none">
      {/* Top Header Control Bar */}
      <div className="h-9 bg-background-base border-b border-white/10 flex items-center justify-between px-3 text-xs text-white/80">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 bg-white/10 hover:bg-white/15 text-white border border-white/10 rounded text-[11px] font-semibold transition active:scale-[0.96] disabled:opacity-50"
            title="Luminar AI 修图"
          >
            <Sparkles className={`w-3.5 h-3.5 ${isLuminarRunning ? 'animate-spin' : ''}`} />
            <span>{isLuminarRunning ? '运行中...' : 'Luminar AI 修图'}</span>
          </button>
          ...
          {/* Sync Zoom indicator */}
          <div
            className="flex items-center gap-1 px-2 py-0.5 bg-white/10 text-white border border-white/20 rounded text-[11px] font-medium"
            title="锁定同步缩放"
          >
      ...
      >
        ...
        <div className="relative flex-1 bg-background-surface overflow-hidden flex items-center justify-center border-r border-border-subtle">
          {beforeSrc && (
            <img
              ref={leftImgRef}
              src={beforeSrc}
          ...
          {/* Before Badge */}
          <div className="absolute top-3 left-3 bg-background-base/80 backdrop-blur-xl px-3 py-1.5 rounded-lg text-xs text-white/80 flex items-center gap-2 border border-white/15 select-text">
        ...
        <div className="relative flex-1 bg-background-surface overflow-hidden flex items-center justify-center">
          {afterSrc && (
            <img
              ref={rightImgRef}
              src={afterSrc}
          ...
          {/* After Badge */}
          <div className="absolute top-3 right-3 bg-background-base/80 backdrop-blur-xl px-3 py-1.5 rounded-lg text-xs text-white/80 flex items-center gap-2 border border-white/15 select-text">
            {hasLuminarEdit ? (
      ...
      <div
        ...
        className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-background-base/80 backdrop-blur-xl px-4 py-2 rounded-lg flex items-center gap-3 border border-white/15 pointer-events-auto opacity-70 hover:opacity-100 transition-opacity duration-200"
      >
        {/* Rating Stars */}
        ...
        <div className="flex items-center gap-1.5" role="group" aria-label="保留或淘汰标记">
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1 rounded-md text-xs font-semibold active:scale-[0.96] transition border ${
              group.flag === 'pick'
                ? 'bg-emerald-600/30 text-emerald-500 border-emerald-500/50'
                : 'bg-transparent text-white/80 hover:bg-white/20 border-transparent'
            }`}
            title="保留 (Pick)"
          >
